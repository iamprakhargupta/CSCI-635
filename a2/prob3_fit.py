...
trial_name = 'prob3' # will add a unique sub-string to output of this program
...
def bernoulli_log_likelihood(p, y):
	...
	# WRITEME: write your code here to complete the routine
	size=y.shape[0]
	loss=np.sum(-np.dot(y.T,np.log(p))-np.dot((1-y).T,np.log(1-p)))/size
	return loss
    ############################################################################
...
def computeCost(X, y, theta, beta): ## loss is now Bernoulli cross-entropy/log likelihood
	...
	loss=bernoulli_log_likelihood(p,y)
...
if  halt <=eps:
	...
	print("Convergence happened at epoch ",i)
############################################################################
...
############################################################################
plt.savefig(outputpath+trial_name+"_output_scatter_with_decision_boundary"+'.jpeg')
plt.show()
